...
async def test_packet_delivery_under_load(setup_dif):
    ...
    load_levels = [1, 3, 5, 7]  # Number of concurrent flows
    results = {}
...
async def test_error_recovery(setup_dif):
    ...
    flow = ipcp1.flows[flow_id]
    flow.timeout = 0.2  # Shorter timeout for faster recovery testing
    flow.retries = 3
    ...
    for i in range(test_packets):
        ...
        # Simulate packet loss by temporarily blocking ACKs
        if i % 4 == 0 and i > 0:
            # Simulate network issue
            original_send_ack = flow.send_ack
            flow.send_ack = lambda seq_num: None  # Block ACKs
